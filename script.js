...
function addCurrentProductToCart(){
  ...
  const colorBg = selectedColor ? (selectedColor.style.background || selectedColor.style.backgroundColor || '#184835') : '#184835';
  const sizeLabel = selectedSize ? selectedSize.textContent.trim() : 'M';
  const item = {
    id: btn.dataset.productId + '_' + sizeLabel + '_' + colorBg.replace(/[^a-z0-9]/gi,''),
...
function renderTotals(){
  ...
  document.getElementById('cartTotal').textContent = fmtEGP(Math.max(0, subtotal - discount));
  const discountRow = document.getElementById('discountRow');
  if (discount > 0) {
    discountRow.style.display = 'flex';
    document.getElementById('cartDiscount').textContent = '— ' + fmtEGP(discount);
  } else {
  ...
  if (activePromo && promoInput && !promoInput.value) {
    ...
    promoMsg.textContent = (body.classList.contains('rtl') ? 'تم تطبيق الكود ✦ ' : 'Code applied ✦ ') + PROMO_CODES[activePromo.code].label;
...
function applyPromo(){
  ...
  if (PROMO_CODES[code]) {
    activePromo = { code };
    savePromo();
    msg.textContent = (isAr ? 'تم تطبيق الكود ✦ ' : 'Code applied ✦ ') + PROMO_CODES[code].label;
...
function checkoutWhatsApp(){
  ...
  if (activePromo && PROMO_CODES[activePromo.code]) {
    const p = PROMO_CODES[activePromo.code];
    discount = p.type === 'percent' ? subtotal * (p.value / 100) : Math.min(p.value, subtotal);
    promoLine = (isAr ? '🌿 كود الخصم: ' : '🌿 Promo Code: ') + activePromo.code +
                ` (-${fmtEGP(discount)})\n`;
  }
  ...
  const itemLines = cart.map((it, idx) => {
